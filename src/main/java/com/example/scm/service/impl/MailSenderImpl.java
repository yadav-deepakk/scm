...
import com.example.scm.service.interfaces.MailingService;

import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
public @Service class MailSenderImpl implements MailingService {
	...
	public boolean sendEmailVerificationMail(User user) throws Exception {
		...
		String LINK = AppConstants.EMAIL_VERIFICATION_BASE_URL + "/auth/verify-email?nonce=" + nonce + "&email="
				+ user.getEmail() + "&otp=" + sixDigitOTP;
	...
	public boolean sendForgetPasswordLink(User user) throws Exception {
		...
		user.setNonce(nonce);
		userService.saveUser(user);
		String LINK = AppConstants.FORGET_PASS_BASE_URL + "/auth/reset-pass?nonce=" + nonce + "&email="
				+ user.getEmail() + "&otp=" + sixDigitOTP;
