...
public @Controller class AuthController {
		...
		try {
			...
			if (verifDelay < AppConstants.EMAIL_VERIFICATION_ALLOWED_DELAY_MIN) {
				if (nonce.equals(user.getNonce()) && otp.equals(user.getEmailOTP())) {
					...
					session.setAttribute("message", Message.builder().messageType(MessageType.green)
							.messageContent(AppConstants.EMAIL_VERIF_SUCCESS).build());
					...
					session.setAttribute("message", Message.builder().messageType(MessageType.red)
							.messageContent(AppConstants.EMAIL_VERIF_ERR_INVALID_LINK).build());
					log.info("Email verification failed due to invalid link for user :: {}", user.getEmail());
				}
			} else {
				session.setAttribute("message", Message.builder().messageType(MessageType.red)
						.messageContent(AppConstants.EMAIL_VERIF_ERR_EXPIRED_LINK).build());
			...
			e.printStackTrace();
			session.setAttribute("message", Message.builder().messageType(MessageType.red)
					.messageContent("Unknown error in email verification.").build());
		}
	...
	@PostMapping("/forget-pass-handler")
	public String forgetPasswordHandler(@Valid @ModelAttribute ForgetPassForm forgetPassForm,
		...
		try {
			...
			session.setAttribute("message", Message.builder().messageType(MessageType.green)
					.messageContent(AppConstants.FORGET_PASS_EMAIL_SUCCESS).build());
		} catch (Exception e) {
			log.info("Error occured while sending forget pass link :: {}", e.getMessage());
			e.printStackTrace();
			session.setAttribute("message", Message.builder().messageContent(AppConstants.FORGET_PASS_ERR_EMAIL_SENDING)
					.messageType(MessageType.red).build());
		}
		return "user/forget-pass.html";
	}
	...
	@PostMapping("/reset-pass-handler")
	public String resetPasswordHandler(@RequestParam String nonce, @RequestParam String email, @RequestParam String otp,
			@Valid @ModelAttribute("resetPasswordForm") ResetPasswordForm form, BindingResult rBindingResult,
			HttpSession session
		...
		if (rBindingResult.hasErrors()) {
			...
			session.setAttribute("message", Message.builder().messageType(MessageType.red)
					.messageContent(AppConstants.RESET_PASS_SUCCESS).build());
			return "user/reset-pass.html";
		}
